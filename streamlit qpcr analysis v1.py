...
class ReportGenerator:
    """The shared Plotly-figure renderer. NOT a slide builder any more.

    This class used to hold a second, from-scratch PowerPoint generator
    (`create_presentation` plus five `_add_*_slide` builders). It had no call
    site in the app -- `PPTGenerator` is what the Export tab uses -- and it
    never received the correctness work that landed on the template path, so it
    had no Cosmax template and, more seriously, a 효능 有/無 verdict that
    ignored `expected_direction`. Re-enabling it would have reintroduced
    exactly the defect that was fixed. It was removed rather than repaired;
    `git log` has it if the dual-gene / grid / summary layouts are ever wanted
    as a starting point for a real template-based feature.

    `_fig_to_image` stays because six live call sites use it.
    """
